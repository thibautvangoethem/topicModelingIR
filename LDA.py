...
def LDA(alpha, beta, nb_documents, words_per_document):
    ...
    for i in range(len(alpha)):  # len(alpha) is equal to the number of topics
        topic_distributions.append(dirichlet(beta))

    document_topic_distributions = []
    for i in range(nb_documents):
        document_topic_distributions.append(dirichlet(alpha))
    ...
    for i in range(nb_documents):
        ...
        for j in range(words_per_document):
            topic = choices(choiceList_topics, document_topic_distribution)[0]
            word = choices(choiceList_words, topic_distributions[topic])[0]
            corpus[i, word] += 1
    print("done")
    return corpus

#LDA([1/3]*3, [1/100]*10, 20, 10)
LDA([1/10]*10, [1/10000]*10000, 1000, 10)
